      ...
      {eventsForDay.map((event, i) => (
        <div
          ...
          style={{
            ...
            right: '10%',
            height: 4,
            backgroundColor: event.extendedProps.isException ? 'red' : '#1976d2',
            borderRadius: 2,
          }}
          title={event.title}
        />
...
return filteredSchedules
  ...
  .map((s, idx) => (
    <div
      key={idx}
      className="border p-3 rounded mb-2 cursor-pointer hover:bg-[#181AD6] hover:text-white"
      onClick={() => {
        setSelectedEvent(event);
        setSelectedSchedule(s);
...
<img src={`https://waw.com.tn${event.business.imageUrl}`} alt={event.business.nom} />
      ...
      <BottomStickyBar>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <Typography variant="body1" sx={{ fontWeight: 600, color: '#3c71c2' }}>
