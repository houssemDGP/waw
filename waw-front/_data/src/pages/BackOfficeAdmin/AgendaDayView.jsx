...
const AgendaDayView = () => {
  ...
  useEffect(() => {
    console.log('--- useEffect: Démarrage de la récupération des réservations ---');
    const fetchReservations = async () => {
      ...
      try {
        ...
        const { grouped, flat } = transformApiReservations(data);
        setAllReservations(grouped);
        setFlatReservations(flat);
        console.log('Réservations transformées et groupées (allReservations):', grouped);
        console.log('Réservations transformées à plat (flatReservations):', flat);
      } catch (e) {
        console.error("Erreur lors de la récupération des réservations:", e);
        setError("Failed to load reservations. Please try again later.");
      } finally {
        setLoading(false);
        console.log('--- useEffect: Fin de la récupération des réservations ---');
      }
    };
    fetchReservations();
  }, []); // Empty dependency array means this runs once on mount

const transformApiReservations = (apiData) => {
  console.log('transformApiReservations: Début de la transformation des données API.');
  const groupedReservations = {};
  const flatReservationsArray = [];

  apiData.forEach(res => {
    // No skip condition here, we'll try to process everything
    const activity = res.event && res.event.nom ? res.event.nom : 'Activité inconnue';
    const date = res.date ? dayjs(res.date) : dayjs(); // Default to today if date is missing
    const formattedDate = date.format('YYYY-MM-DD');
    const clientName = res.nomClient || 'Client Inconnu';
    const startTime = res.dailyScheduleReservation && res.dailyScheduleReservation.startTime ? res.dailyScheduleReservation.startTime : 'N/A';
    const endTime = res.dailyScheduleReservation && res.dailyScheduleReservation.endTime ? res.dailyScheduleReservation.endTime : 'N/A';
    const capacity = res.formule && res.formule.capacity ? res.formule.capacity : 0;
    const numPeople = res.personnes ? res.personnes.length : 1;
    ...
    const transformedRes = {
      ...
      statut: 'Confirmée', // Defaulting status
      paiement: res.paymentMethods && res.paymentMethods.length > 0 ? res.paymentMethods : 'Non spécifié',
      max: capacity,
      quantity: numPeople,
      table: activity === 'Beach Bar' && res.client && res.client.includes(' – Table ') ? res.client.split(' – ')[1] : undefined,
      originalDate: date,
    };
  ...
  console.log('transformApiReservations: Transformation terminée.');
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <AgendaContainer>
            <LeftPane>
              ...
              <Box sx={{ width: isMobile ? '100%' : 'auto', mb: 2 }}>
                <ToggleButtonGroup
                  value={view}
                  exclusive
                          ...
                          <Accordion key={activity} defaultExpanded sx={{ mb: 2 }}>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                              <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', pr: 2 }}>
                                ...
                                <IconButton onClick={(e) => { e.stopPropagation(); handleEditActivity(activity); }} size="small" sx={{ mr: 1 }}>
                                  <EditIcon />
                                </IconButton>
                                <IconButton onClick={(e) => { e.stopPropagation(); handleDeleteActivity(activity); }} size="small">
                                  <DeleteIcon />
                                </IconButton>
                              </Box>
                            </AccordionSummary>
                                    ...
                                    {reservationsForSelectedDate.length > 0 ? (
                                      reservationsForSelectedDate.map((res, idx) => {
                                        ...
                                        <TableRow key={idx}>
                                          ...
                                          <TableCell>
                                            <IconButton onClick={() => handleEditReservation(res)} size="small">
                                              <EditIcon />
                                            </IconButton>
