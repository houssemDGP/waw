...
const AppHeader: React.FC = () => {
  ...
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      const gmtString = now.toUTCString(); // ex: "Mon, 29 Sep 2025 08:15:30 GMT"
      const [dayName, day, month, year, time] = gmtString.split(" ");

      setDateTime({
        time,                                 // "08:15:30"
        date: `${day} ${month} ${year}`       // "29 Sep 2025"
      });
    };
...
<h3>{data?.rs || "WHEN AND WHERE"}</h3>
          </div>
        </div>
        <div
          className={`${
            isApplicationMenuOpen ? "hidden" : "flex"
          } items-center justify-between w-full gap-4 px-5 py-4 lg:flex shadow-theme-md lg:justify-end lg:px-0 lg:shadow-none`}
        >
          <div className="flex items-center gap-2 2xsm:gap-3">
    <div className="bg-[#021732] text-white text-xs md:text-sm px-3 py-1 rounded-lg font-mono">
      <span>{dateTime.date}</span>
      <span> GMT {dateTime.time}</span>    </div>                      <UserDropdown />
