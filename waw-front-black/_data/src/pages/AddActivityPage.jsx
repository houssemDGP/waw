...
function LocationSelector({ position, setPosition, setFormData }) {
  const markerRef = React.useRef(null);
  
  // Fonction pour le reverse geocoding
  const reverseGeocode = async (lat, lng) => {
    try {
      ...
      if (response.data) {
        ...
        return {
          rue: response.data.display_name || "",
          ville: address.city || address.town || address.village || address.municipality || "",
  ...
  // Met à jour la position du marker quand la prop position change
  React.useEffect(() => {
  ...
  const handlePositionUpdate = async (newPosition) => {
  ...
  useMapEvents({
    async click(e) {
      ...
      await handlePositionUpdate(newPosition);
    },
  });
...
const handleAddDailySchedule = (rangeIndex) => {
  ...
  const paymentOptions = ['Carte bancaire', 'Espèces','Virement', 'Chèque','Gratuit','Autres'];
...
const steps = [
  ...
  { label: "Ajouter des date et heures", optional: false },
...
function addDailySchedule(rangeIndex) {
  setScheduleRanges((prev) => {
    ...
    const schedules = copy[rangeIndex].dailySchedules;
    const newSchedule = schedules.length > 0
      ? {
          ...schedules[schedules.length - 1],
          formulas: schedules[schedules.length - 1].formulas.map(f => ({ ...f })),
        }
      : {
          startTime: "",
          endTime: "",
    ...
    copy[rangeIndex] = {
      ...copy[rangeIndex],
      dailySchedules: [...schedules, newSchedule],
    };
  ...
  function removeFormula(rangeIndex, scheduleIndex, formulaIndex) {
    const copy = [...scheduleRanges];
    if (copy[rangeIndex].dailySchedules[scheduleIndex].formulas.length <= 0) return;
...
const [ageMaxBebe, setAgeMaxBebe] = useState(0);
const [ageMaxEnfant, setAgeMaxEnfant] = useState(0);
...
const handleSearch = async () => {
  ...
  try {
    ...
    if (response.data.length > 0) {
      ...
      setPosition(newPosition);
      setFromSearch(true); // ✅ this tells MapContainer to re-render
      ...
      console.warn("No location found for your query");
    }
  } catch (error) {
...
return (
<>      <PageBreadcrumb pageTitle="Ajouter une activité" />
      <div className="space-y-6">
      <PageMeta title="Ajouter une activité" description="Ajouter une activité" />
...
<RadioGroup
  ...
  onChange={(e) => setAccepteEnfants(e.target.value === 'true')}
>
...
{accepteEnfants && (
  <>
    <Typography variant="body2">Âge maximum enfant</Typography>
    <TextField
      type="number"
      placeholder="Ex: 12"
      value={ageMaxEnfant}
      onChange={(e) => setAgeMaxEnfant(e.target.value)}
      size="small"
      sx={{ width: 80 }}   // 👈 largeur compacte
    />
    </>
)}
  </Box>
  <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: 1 }}>
                      ...
                      <RadioGroup row
  value={accepteBebes}
  onChange={(e) => setAccepteBebes(e.target.value === 'true')}
                      >
 <FormControlLabel value={true} control={<Radio />} label="Oui" />
  <FormControlLabel value={false} control={<Radio />} label="Non" />
                      </RadioGroup>
                                             {accepteBebes && (
<>    <Typography variant="body2">Âge maximum bébé</Typography>
    <TextField
      fullWidth
      type="number"
      placeholder="Ex: 2"
      value={ageMaxBebe}
      onChange={(e) => setAgeMaxBebe(e.target.value)}
      size="small"
      sx={{ width: 80 }}
    />
</>)}                 
      ...
      >
          ...
          .map((act) => (
            <Button
              ...
              sx={{
                flex: "0 0 auto",
                borderColor: "black",
                color: activiteSelected?.titre === act.titre ? "#fff" : "black",
                backgroundColor:
                  activiteSelected?.titre === act.titre ? "black" : "transparent",
                "&:hover": {
                  backgroundColor: "black",
                  borderColor: "black",
                },
        ...
        <Button
          ...
          sx={{
            flex: "0 0 auto",
            borderColor: "black",
            color: showCustomInput ? "#fff" : "black",
            backgroundColor: showCustomInput ? "black" : "transparent",
            "&:hover": {
              backgroundColor: "black",
              borderColor: "black",
            },
...
{showCustomInput && (
  ...
  const newAct = response.data; // ✅ contient id, titre, etc.
  setListactivites((prev) => [...prev, newAct]);
  setActiviteSelected(newAct);  // ✅ sélection correcte

  setCustomActivite("");
  setShowCustomInput(false);

  // Peut même être inutile si on ajoute déjà manuellement :
  // fetchActivities();
} catch (err) {
  console.error(err);
...
<Box sx={{ height: 300, mb: 2, borderRadius: 2, overflow: "hidden" }}>
  <MapContainer
    ...
    key={fromSearch ? `${position[0]}-${position[1]}` : undefined} // ✅ re-render ONLY after search
  >
    <TileLayer
...
{imagesFiles.length > 0 && (
  ...
  <div className="flex items-center space-x-2">
    ...
    />
    <button
      onClick={handleAddInstagramVideo}type="button"
...
<Autocomplete
  ...
  renderInput={(params) => (
    <TextField {...params} placeholder="Pour ajouter un moyen de paiement cliquer ici"      error={Boolean(errors.selectedPayments)}
      helperText={errors.selectedPayments}/>
  )}
/>
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField {...params} placeholder="Pour ajouter une langue cliquer ici"      error={Boolean(errors.languages)}
  ...
  onChange={(e) => setCgv(e.target.value)}
    error={Boolean(errors.cgv)}
  helperText={errors.cgv}
