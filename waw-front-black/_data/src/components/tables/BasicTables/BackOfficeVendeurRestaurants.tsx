...
function RestaurantDetailModal({ isOpen, onClose, restaurant }) {
...
export default function BackOfficeVendeurRestaurants() {
  ...
  useEffect(() => {
    async function fetchRestaurants() {
      try {
        const res = await fetch(`https://waw.com.tn/api/api/restaurants/business/${businessId}`);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-800 dark:text-white">Liste des restaurants</h2>
        <Link to="/vendeur/ajouter-restaurant">
...
<div className="flex items-center gap-2 flex-wrap">
  ...
  {/* Éditer */}
  <button
    onClick={() => window.location.href = `/vendeur/restaurant/edit/${restaurant.id}`}
  ...
  {/* Gérer les places */}
  <button
    onClick={() => window.location.href = `/vendeur/restaurant/${restaurant.id}/places`}
